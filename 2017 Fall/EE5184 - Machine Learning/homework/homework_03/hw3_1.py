...
def main():
	X_all,Y_all,X_test,X_train,X_valid,Y_train,Y_valid = load(False)
	...
	#a = pd.DataFrame(Y_train).columns.values
	#a = pd.DataFrame(Y_valid).groupby(0)[0].count()
	'''
	...
	filepath="weight/weights-improvement-{epoch:02d}-{val_acc:.3f}.hdf5"
